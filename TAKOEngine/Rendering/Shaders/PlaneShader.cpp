...
/**************************************************************************//**
	...
	@param[in]    vertexShaderName	頂点シェーダーパス
...
PlaneShaderDX12::PlaneShaderDX12(ID3D12Device* device, const char* vertexShaderName, const char* pixelShaderName)
{
	...
	_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	d3d_cbv_resource->SetName(L"PlaneConstatBuffer");
...
/**************************************************************************//**
	...
	@param[in]    pixelShaderName	ピクセルシェーダパス
	@param[in]    geometryShaderName	ジオメトリーシェーダーパス
*//***************************************************************************/
...
void BillBoardShaderDX12::Render(const RenderContextDX12& rc, const ModelDX12::Mesh& mesh)
{
	...
	// シーン定数バッファ設定
	rc.d3d_command_list->SetGraphicsRootDescriptorTable(1, rc.plane_cbv_descriptor->GetGpuHandle());
...
/**************************************************************************//**
	...
	@param[in]    pixelShaderName	ピクセルシェーダパス
*//***************************************************************************/
MagicPlaneDX12::MagicPlaneDX12(ID3D12Device* device, const char* vertexShaderName, const char* pixelShaderName)
{
	Graphics& graphics = Graphics::Instance();
	const RenderStateDX12* renderState = graphics.GetRenderStateDX12();
	...
	// パイプラインステートの生成
	{
		...
		//深度ステンシルステート
		pipeline_state_desc.DepthStencilState = renderState->GetDepthState(DepthState::TestOnly);
		...
		hr = device->CreateGraphicsPipelineState(
			...
			IID_PPV_ARGS(m_d3d_pipeline_state.GetAddressOf()));
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
		m_d3d_pipeline_state->SetName(L"MagicPlanePipelineState");
	}

	m_sampler = graphics.GetSampler(SamplerState::AnisotropicWrap);
}

void MagicPlaneDX12::Render(const RenderContextDX12& rc, const ModelDX12::Mesh& mesh)
{
	Graphics& graphics = Graphics::Instance();
