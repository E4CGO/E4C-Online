...
//**********************************************************************
// @brief     フィルター値計算
// @param[in] cbFilter  コンスタントバッファに送るデータ
...
// @return    なし
//**********************************************************************
GaussianBlurShaderDX12::GaussianBlurShaderDX12(ID3D12Device* device)
{
	...
	{
		GpuResourceUtils::LoadShaderFile("Data/Shader/GaussianBlurDX12VS.cso", vsData);
		GpuResourceUtils::LoadShaderFile("Data/Shader/GaussianBlurDX12PS.cso", psData);
	}
	...
	// ルートシグネクチャの生成
	{
		...
		m_d3d_root_signature->SetName(L"GaussianBlurShaderRootSignature");
	}
	...
	// パイプラインステートの生成
	{
		...
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
		m_d3d_pipeline_state->SetName(L"GaussianBlurShaderPipelineState");
	}
}
...
// @return      なし
//***********************************************************
void GaussianBlurShaderDX12::Render(const RenderContextDX12& rc, SpriteDX12* sprite)
{
	Graphics& graphics = Graphics::Instance();
