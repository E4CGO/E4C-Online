...
/**************************************************************************//**
	...
	@brief		四分木空間マネージャー
	@par		四分木空間ノードを線形上に並べ、様々な処理を執り行う
*//***************************************************************************/
class QuadtreeNodeManager
{
public:
...
public:
	...
	// 四分木空間へオブジェクトを登録する際の線形インデックスを取得
	...
	private:
		std::vector<QuadtreeNode>	m_quadtreeNodes = {};	// 四分木ノード配列
		uint32_t	m_depth = 0;	// 分割数

		static const uint32_t	MAX_DEPTH = 10u;	// 最大分割数
		static constexpr uint32_t LevelStart[MAX_DEPTH] =	// 線形四分木の各階層の開始インデックス。等比数列の和の公式を利用することで求まるが定数なのであらかじめ計算しておく
		{
